...
export default function BlogGenerationPage() {
  ...
  const handleGenerate = async (topic: string, tone: string, length: number) => {
    ...
    try {
      ...
      setChatMessages([{
        role: 'ai',
        content: `Great! I've created an outline for your blog post about "${topic}" with a ${tone.toLowerCase()} tone.\n\nThe structure includes ${data.outlines_json.sections.length} main sections covering introduction, key concepts, current trends, practical applications, future outlook, and conclusion.\n\n${data.follow_up_question}`,
        time: getCurrentTime(),
      }]);
    } catch (error) {
